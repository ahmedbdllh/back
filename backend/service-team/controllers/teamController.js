...
const fetchUserData = async (userIds) => {
  try {
    ...
    const response = await axios.post('http://localhost:5000/api/auth/users/bulk', {
      userIds: userIds
    });
...
exports.getMyTeam = async (req, res) => {
  try {
    ...
    // Find team where user is a member or captain
    const team = await Team.findOne({
      $or: [
        { captain: userId },
        { 'members.user': userId }
...
// Make an offer to another team for instant play
exports.makeOffer = async (req, res) => {
  try {
    ...
    if (!targetTeamId || !proposedTime || !proposedDate || !sport) {
      return res.status(400).json({ 
        error: 'Missing required fields: targetTeamId, proposedTime, proposedDate, sport' 
      });
    }
    ...
    if (!userTeam) {
      return res.status(404).json({ error: 'You must be part of a team to make offers' });
    }
    ...
    // Check if user is trying to offer to their own team
    if (userTeam._id.toString() === targetTeamId) {
      return res.status(400).json({ error: 'Cannot make offer to your own team' });
    }

    // Check if sports match
    if (userTeam.sport.toLowerCase() !== sport.toLowerCase()) {
      return res.status(400).json({ error: 'Sport mismatch with your team' });
    }

    if (targetTeam.sport.toLowerCase() !== sport.toLowerCase()) {
      return res.status(400).json({ error: 'Sport mismatch with target team' });
    }
    ...
    const offer = {
      ...
      court: court || 'TBD',
      sport,
      message: message || `${userTeam.name} would like to play against your team`,
      createdAt: new Date(),
      status: 'pending'
    };

    // Add offer to target team's offers array (or create notification)
    if (!targetTeam.offers) {
      targetTeam.offers = [];
    }
    ...
    console.log(`📨 Offer sent from ${userTeam.name} to ${targetTeam.name} for ${proposedDate} at ${proposedTime}`);

    res.json({ 
      message: 'Offer sent successfully',
      offer: {
        fromTeam: offer.fromTeam.name,
        toTeam: offer.toTeam.name,
        proposedTime,
        proposedDate,
        court: offer.court,
        sport
...
// Get offers received by user's team
exports.getTeamOffers = async (req, res) => {
  try {
    const userId = req.user.userId;
...
// Instant search for available teams
exports.instantSearch = async (req, res) => {
  try {
    const { date, time, court, sport } = req.query;

    // Validate required parameters
    if (!time || !sport) {
      return res.status(400).json({ 
        error: 'Missing required parameters: time and sport' 
      });
    }

    console.log(`🔍 Instant search request: sport=${sport}, time=${time}, date=${date || 'today'}`);

    // Build search criteria
    const searchCriteria = {
      sport: { $regex: new RegExp(sport, 'i') }, // Case-insensitive sport match
      'settings.isPublic': true // Only show public teams for now
    };

    // Find teams matching the criteria
    const teams = await Team.find(searchCriteria)
      .select('name sport description logo members maxMembers createdAt settings')
      .limit(20) // Limit results for performance
      .sort({ createdAt: -1 });

    console.log(`🔍 Instant search for ${sport} at ${time}: Found ${teams.length} teams`);
    ...
    console.error('❌ Error in instant search:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.acceptOffer = async (req, res) => {
  try {
    ...
    team.offers[offerIndex].status = 'accepted';
    team.offers[offerIndex].respondedAt = new Date();
...
exports.declineOffer = async (req, res) => {
  try {
    ...
    // Check if user is captain (only captains can decline offers)
    if (team.captain.toString() !== userId) {
      return res.status(403).json({ error: 'Only team captains can decline offers' });
    }
    ...
    team.offers[offerIndex].status = 'declined';
    team.offers[offerIndex].respondedAt = new Date();
    
    await team.save();

    console.log(`❌ Offer declined by ${team.name} from ${offer.fromTeam.name}`);
