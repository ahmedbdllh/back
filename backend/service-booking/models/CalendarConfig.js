...
const calendarConfigSchema = new mongoose.Schema({
  ...
  pricing: {
    basePrice: { type: Number, required: true, default: 50 }, // Price per hour
    peakHours: [{
...
calendarConfigSchema.methods.calculatePrice = function(date, startTime, endTime) {
  ...
  return Math.round(basePrice * multiplier * durationHours * 100) / 100; // Round to 2 decimal places
};
